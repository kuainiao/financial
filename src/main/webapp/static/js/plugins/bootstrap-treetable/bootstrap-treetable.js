 ...
 * @param options 存放了一些常量，例如展开和收缩的classtoolbarId
 */
...
(function ($) {
    ...
    $.fn.bootstrapTreeTable = function (options, param) {
        ...
        target.renderRow = function (item, isP, lv) {
            ...
            } else if(!options.expandAll && lv > 1){
                tr.css("display", "none");
        ...
        target.load = function (parms) {
            ...
            $.each(options.columns, function (i, item) {
                ...
                if (i == 0 && item.field == 'selectItem') {
                    hasSelectItem = true;
                    if(item.checkbox){
                        th = $('<th style="width:36px"><input name="selectAll" type="checkbox"/></th>');
                    }else{
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    target.find("tbody").find("tr").dblclick(function () {
                        if (hasSelectItem) {
                            ...
                            if (_ipt.attr("type") == "radio") {
                                ...
                                var checkboxVal = _ipt.val();
                                var checkboxs = $(target.find("tbody").find("tr")).find("input[name='select_item']");
                                for(var i=0;i<$(checkboxs).length;i++){
                                    if($(checkboxs[i]).val() == checkboxVal){
                                        if($(checkboxs[i]).prop('checked')){
                                            $(checkboxs[i]).prop('checked', false);
                                            $($($(checkboxs[i]).parent()).parent()).removeClass("treetable-selected");
                                        }else{
                                            $(checkboxs[i]).prop('checked', true);
                                            $($($(checkboxs[i]).parent()).parent()).addClass("treetable-selected");
                                        }
                                    }
                                }
                            }
                    ...
                    //复选框点击事件
                    $(target.find("tbody").find("tr")).find("input[name='select_item']").change(function () {
                    ...
                    //全选事件
                    $(target.find("thead").find("th")).find("input[name='selectAll']").change(function () {
                        var _ipt = $(target.find("tbody").find("td")).find("input[name='select_item']");
                        var isCheck = $(this).is(':checked');
                        if (isCheck) {
                            _ipt.prop('checked', true);
                            _ipt.parent().parent().addClass("treetable-selected");
                        } else {
                            _ipt.prop('checked', false);
                            _ipt.parent().parent().removeClass("treetable-selected");
                        }
                    });
                    // 小图标点击事件--展开缩起
